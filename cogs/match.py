...
async def simular_partido(interaction, club_a_id, nombre_a, club_b_id, nombre_b):
    ...
    for minuto in [15, 30, 45, 60, 75, 90]:
        await asyncio.sleep(2)

        # Selección de evento
        tipo_evento = random.choices(
            list(EVENTOS_NARRATIVA.keys()),
            weights=[0.05, 0.15, 0.20, 0.10, 0.50]
        )[0]

        # Determinar quién protagoniza el evento basado en medias
        # Si media_a es mayor, tiene más probabilidad de ser el protagonista
        ...
        if tipo_evento == "gol":
            # El equipo con más media tiene más probabilidad de meter el gol cuando tiene la ocasión
            if random.random() < (media_a / (media_a + media_b) if protagonista_a else media_b / (media_a + media_b)):
                ...
                texto = f"¡Gran ocasión de {jugador}, pero el portero lo evita!"
    ...
    await interaction.followup.send(f"🏁 **Resultado final:** {nombre_a} {goles_a} - {goles_b} {nombre_b}")
...
class MatchCog(commands.Cog):
    ...
    async def partido(self, interaction: discord.Interaction):
        # Obtener ID del club del usuario
        club_usuario_id = obtener_club_id_por_usuario(interaction.user.id)

        if not club_usuario_id:
            await interaction.response.send_message("❌ Primero debes crear un club usando `/crear_club`.",
                                                    ephemeral=True)
            return

        # Obtener datos del rival IA
        ...
        # Iniciar simulación
        await interaction.response.defer()  # El bot está "pensando"
