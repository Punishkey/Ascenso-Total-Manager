...
async def mostrar_ficha_jugador(interaction: discord.Interaction, club_id: int, numero: int):
    ...
    if not jugador:
        if not interaction.response.is_done():
            await interaction.response.send_message(f"❌ No se encontró ningún jugador con el dorsal #{numero}.", ephemeral=True)
        return
    ...
    embed.add_field(name="Mentalidad", value=f"📈 Potencial: {pot} | 🧠 Prof: {prof} {obtener_icono_progreso(prof, prof_b)}", inline=False)
...
class FichaJugadorView(discord.ui.View):
    ...
    async def configurar_precio(self, interaction: discord.Interaction, _button: discord.ui.Button):
        ...
        if tiene_tienda_merchandising(self.club_id) < 1:
            await interaction.response.send_message("❌ Necesitas tienda nivel 1.", ephemeral=True)
            return
        await interaction.response.send_modal(ConfigurarPrecioModal(self.club_id, self.media, jugador_id=self.jugador_id))